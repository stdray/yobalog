...
- [ ] **Фаза D — KQL-first UI + админка.** Интерфейс, который заточен под KQL вместо формы фильтров, и DB-backed CRUD для админских сущностей вместо конфига.
    ...
    - [~] **Админка через UI.** Сейчас всё config-driven: workspaces косвенно через `ApiKeys.Keys[].Workspace`, API-ключи plain в `ConfigApiKeyStore`, retention policies массивом в `Retention.Policies`. Переезжаем на DB-бэкед стораджи в `$system.meta.db`:
        ...
        - `IApiKeyStore` DB-backed (заменяет `ConfigApiKeyStore`): хранить `sha256(token)` + первые 6 символов для UI-идентификации, plaintext показывать **один раз** при создании. Миграция config → DB при старте, если DB пустая. В UI API-ключа кнопка **«Copy Seq URL»** — копирует готовый base URL `https://<host>/compat/seq` (+ пояснение, что в Serilog/winston/seqlog надо указать его как `serverUrl`, клиент сам допишет `/api/events/raw`).
