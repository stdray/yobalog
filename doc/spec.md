...
- **Admin:**
    ...
    - **Auth surface:** cookie-сессия для UI (`/Login` → cookie) + personal admin tokens для scripting через `/v1/admin/*` JSON CRUD. Token-tier — `AdminTokens` таблица в `$system.meta.db`, per-user multi-token, plaintext-once, soft-delete = revoke (`/admin/profile`). Cascade hard-delete токенов при `IUserStore.DeleteAsync`. Транспорты: `Authorization: Bearer`, `X-YobaLog-AdminToken`, `?adminToken=`; конфликт двух разных значений → `400 ambiguous_auth`. Эндпоинты: `/v1/admin/workspaces`, `/v1/admin/workspaces/{ws}/api-keys`, `/v1/admin/workspaces/{ws}/retention`. Полная справка — `doc/admin-api.md`. Decision-log 2026-04-28.
