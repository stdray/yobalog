...
5a. **`/health` 302 → `/Login` (наблюдалось один раз).** На первом живом деплое 2026-04-21 curl `http://127.0.0.1:8082/health` изнутри VM вернул `302 Found` с `Location: /Login?ReturnUrl=%2Fhealth`, хотя endpoint имеет `.AllowAnonymous()`. **Не воспроизводится локально**: 4 регрессионных теста (`HealthEndpointTests` — TestServer под Testing+Production env + real Kestrel под Production env) проходят зелёным. Гипотезы: (1) curl попал в momentum между старт-стопами контейнеров в deploy-flow когда Caddy ещё proxied на старый container с другой конфигой; (2) какая-то cache/warm-up-specific quirk Kestrel под chiseled runtime который не reproduced on developer machine. **Watch**: если повторится — zaydocать `/health` headers + `journalctl -u caddy` + `docker logs yobalog` + сохранять `Program.cs` endpoint metadata dump через debug endpoint. Tests guarded regression.
