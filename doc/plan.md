...
- [ ] **Фаза D — usability.** Share links + маскирование UI; TSV export; live tail.
    ...
    - [x] **Share → masked TSV** per spec §3. HMAC-подписанный токен несёт `{workspace, KQL, expiry, 16-byte salt, mask-modes}`. `GET /share/{token}.tsv` anonymous, 410 Gone по истечении. Три режима на поле: `keep` / `mask` (детерминированный HMAC-SHA256(salt,value)→4-byte hex с префиксом из последнего сегмента пути — `email:a1b2c3d4`, связи в рамках одной ссылки сохраняются) / `hide` (поле вырезается). Policy-per-workspace в `FieldMaskingPolicy` (в `.meta.db`) — модалка не переспрашивает одно и то же. Rate-limit и отзыв отложены до реального трафика. HTML-preview шарированного view — отложен, 99% use-case — передать TSV LLM-агенту.
